...
def run_tests(student_file, tests_dir):
    ...
    if not hasattr(mod, "dijkstra"):
        return [{"name": "import", "passed": False,
                 "error": "No se encuentra la función dijkstra(g, start)."}]
    ...
    for test_file in test_files:
        ...
        g, n, start = build_graph(raw)

        try:
            result = mod.dijkstra(g, start)
            # Acepta lista o cualquier iterable, ignora índice 0
            got = " ".join(str(x) for x in result[1:])
            passed = got == expected
            results.append({
                "name": test_name,
                "passed": passed,
