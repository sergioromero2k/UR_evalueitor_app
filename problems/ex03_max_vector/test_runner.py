...
def run_tests(student_file, tests_dir):
    ...
    if not hasattr(mod, "max_vector_dyv_efficient"):
        return [{"name": "import", "passed": False,
                 "error": "No se encuentra la funcion max_vector_dyv_efficient(v, l, h)."}]
    ...
    for test_file in test_files:
        ...
        try:
            result = mod.max_vector_dyv_efficient(v, 0, len(v)-1)
            got    = str(result)
            passed = got == expected
            results.append({
                "name":     test_name,
