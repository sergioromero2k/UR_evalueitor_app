...
def run_tests(student_file, tests_dir):
    ...
    if not hasattr(mod, "knapsack_bt"):
        return [{"name": "import", "passed": False,
                 "error": "No se encuentra la funcion knapsack_bt(data, sol, best_sol, k)."}]
    ...
    for test_file in test_files:
        ...
        try:
            best_sol = mod.knapsack_bt(data, sol, best_sol, 0)
            sys.stdout = old_stdout

            valor    = best_sol['v']
            indices  = [i for i, x in enumerate(best_sol['o']) if x > 0]
