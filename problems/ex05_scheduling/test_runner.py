...
def parse_input(raw_lines):
    """Parsea la entrada y devuelve nombres, beneficios y deadlines."""
    n = int(raw_lines[0])
    names    = []
    benefits = []
    deadlines = []
    for i in range(1, n + 1):
        ...
        benefits.append(int(parts[1]))
        deadlines.append(int(parts[2]))
    return names, benefits, deadlines
...
def run_tests(student_file, tests_dir):
    ...
    if not hasattr(mod, "greedy_schedule"):
        return [{"name": "import", "passed": False,
                 "error": "No se encuentra la función greedy_schedule(names, profit, deadline)."}]
    ...
    for test_file in test_files:
        ...
        names, benefits, deadlines = parse_input(raw_lines)
        ...
        try:
            mod.greedy_schedule(names, benefits, deadlines)
