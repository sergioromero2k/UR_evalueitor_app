...
def run_tests(student_file, tests_dir):
    ...
    if not hasattr(mod, "binary_search"):
        return [{"name": "import", "passed": False,
                 "error": "No se encuentra la funcion binary_search(v, number)."}]
    ...
    for test_file in test_files:
        ...
        try:
            result = mod.binary_search(v, number)
            got    = str(result)

            # Acepta: encontrado → positivo correcto
            # No encontrado → cualquier valor negativo
            exp_val = int(expected)
            if exp_val >= 0:
                passed = int(got) == exp_val
            else:
                passed = int(got) < 0
