...
def run_tests(student_file, tests_dir):
    ...
    if not hasattr(mod, "findArticulationPoints"):
        return [{"name": "import", "passed": False,
                 "error": "No se encuentra la función findArticulationPoints(g)."}]
    ...
    for test_file in test_files:
        ...
        try:
            result = mod.findArticulationPoints(g)
            got    = " ".join(str(x) for x in sorted(result))
            passed = got == expected
            results.append({
                "name": test_name,
                "passed": passed,
